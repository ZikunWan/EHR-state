...
INDEX_DIR="/data/zikun_workspace/input/tasks/classification/ehrshot/indices"
...
python preprocess/ehrshot/1_generate_patient_ehr.py \
  ...
  --output_utils_dir "${TABLE_DIR}/utils"
...
python preprocess/ehrshot/generate_tte_task_index.py \
  --root_dir "${TABLE_DIR}" \
  --train_index_path "${INDEX_DIR}/ehrshot_train.csv" \
  --val_index_path "${INDEX_DIR}/ehrshot_val.csv" \
  --test_index_path "${INDEX_DIR}/ehrshot_test.csv" \
