...
class CustomTrainingArguments(TrainingArguments):
    wandb_project: Optional[str] = "eicu-TTE"
...
def build_dataset(args, path, shuffle, limit, max_bins):
    base = EICUDataset(root_dir=args.data_dir, processed_dir=args.processed_dir, sample_info_path=path, task_name=args.task_name, lazy_mode=True, shuffle=shuffle, max_samples=limit)
...
def main():
    ...
    rank0_print(f"Training eicu/{data_args.task_name}: train={len(train_dataset)}, val={len(val_dataset)}, bins={max_bins}")
