...
class CustomTrainingArguments(TrainingArguments):
    wandb_project: Optional[str] = "ehr_bench-TTE"
...
def build_dataset(args, path, shuffle, limit, max_bins):
    base = MIMICIV(root_dir=args.data_dir, sample_info_path=path, lazy_mode=True, shuffle=shuffle, max_samples=limit, use_table_length_cache=False)
...
def main():
    ...
    rank0_print(f"Training ehr_bench/{data_args.task_name}: train={len(train_dataset)}, val={len(val_dataset)}, bins={max_bins}")
