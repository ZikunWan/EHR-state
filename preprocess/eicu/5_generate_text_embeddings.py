"""
Generate pre-computed text embeddings for unique table texts in eICU.
...
def init_harvest_worker(root_dir, processed_dir, sample_info_path):
    global _HARVEST_DATASET
    _HARVEST_DATASET = EICUDataset(
        root_dir=root_dir,
...
def harvest_worker(worker_args):
    ...
    for sample_idx in slice_list:
        ...
        df = dataset.structed_EHR_input_process(sample_info)
...
def harvest_unique_texts(root_dir, processed_dir, splits, harvest_checkpoint, num_workers, num_harvest_chunks):
    os.makedirs(os.path.dirname(harvest_checkpoint), exist_ok=True)
    parts_dir = f"{harvest_checkpoint}.parts"
    os.makedirs(parts_dir, exist_ok=True)

    print("Phase 1: Harvesting unique texts from eICU tables...")
    split_part_counts = {}

    for split in splits:
        sample_info_path = os.path.join(processed_dir, f"sample_info_{split}.json")
        dataset = EICUDataset(
            root_dir=root_dir,
            processed_dir=processed_dir,
            sample_info_path=sample_info_path,
        ...
        for i, sample_info in enumerate(dataset.sample_info):
            key = (sample_info["icustay_id"], sample_info.get("obs_hours", dataset.obs_size))
        ...
        print(
            ...
            f"unique stays/windows={len(slice_indices)}"
        )
        ...
        if pending_args:
            ...
            with Pool(
                ...
                initargs=(root_dir, processed_dir, sample_info_path),
            ) as pool:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Harvest eICU table texts and generate text embeddings."
    )
    parser.add_argument(
        "--stage",
    ...
    parser.add_argument(
        ...
        help="Raw eICU data root.",
    )
    parser.add_argument(
        "--processed-dir",
        type=str,
        default="/data/zikun_workspace/eicu-crd/processed",
        help="Processed eICU directory containing sample_info_*.json and patients/.",
    )
    parser.add_argument(
        "--splits",
        nargs="+",
        default=["train", "val", "test"],
        help="Splits to harvest from processed_dir.",
    )
    parser.add_argument(
        "--model-path",
        type=str,
