"""
Generate pre-computed text embeddings for unique table texts in EHRSHOT.

Usage:
    python preprocess/ehrshot/3_generate_text_embeddings.py --stage harvest
    ...
    torchrun --nproc_per_node=4 preprocess/ehrshot/3_generate_text_embeddings.py --stage encode \
        ...
        --final-output /data/zikun_workspace/input/cache/embeddings/ehrshot/text_embeddings_clinicalbert.pt

    torchrun --nproc_per_node=4 preprocess/ehrshot/3_generate_text_embeddings.py --stage encode \
        --model-path /data/zikun_workspace/checkpoints/pretraining/text_encoder_stage1/epoch_100.pt \
        --final-output /data/zikun_workspace/input/cache/embeddings/ehrshot/text_embeddings_stage1.pt
    ...
    torchrun --nproc_per_node=8 preprocess/ehrshot/3_generate_text_embeddings.py --stage encode \
        ...
        --final-output /data/zikun_workspace/input/cache/embeddings/ehrshot/text_embeddings_stage2.pt
"""
import argparse
import os
import pickle
...
def init_harvest_worker(data_dir, index_path):
    global _HARVEST_DATASET
    _HARVEST_DATASET = EHRSHOTDataset(
        root_dir=data_dir,
...
def harvest_worker(worker_args):
    """Extract unique table strings from a set of sample slices."""
    ...
    for sample_idx in slice_list:
        ...
        if not df.empty:
            local_unique.update(df["Item"].dropna().astype(str).unique())
            local_unique.update(df["Unit"].dropna().astype(str).unique())
            value_texts = df["Value"].dropna().astype(str)
            numeric_values = pd.to_numeric(value_texts, errors="coerce")
            local_unique.update(value_texts[pd.isna(numeric_values)].unique())
    return part_idx, local_unique


def harvest_part_path(parts_dir, part_idx):
    return os.path.join(parts_dir, f"part_{part_idx:06d}.pkl")


def save_harvest_part(parts_dir, part_idx, texts):
    with open(harvest_part_path(parts_dir, part_idx), "wb") as f:
        pickle.dump(texts, f)


def load_harvest_part(parts_dir, part_idx):
    with open(harvest_part_path(parts_dir, part_idx), "rb") as f:
...
def harvest_unique_texts(data_dir, index_file, harvest_checkpoint, num_workers, num_harvest_chunks):
    ...
    for i, sample_info in enumerate(dataset.sample_info):
        key = (
            ...
            sample_info["period_begin"],
            sample_info["period_end"],
        )
    ...
    print(
        ...
        f"Unique slices to process: {len(slice_indices)}"
    )
    ...
    pending_args = [
        ...
        if not os.path.exists(harvest_part_path(parts_dir, part_idx))
    ]

    print(f"Harvest parts: {num_chunks}, pending: {len(pending_args)}")
    ...
    if pending_args:
        ...
        with Pool(
            ...
            for part_idx, result in tqdm(
                ...
                save_harvest_part(parts_dir, part_idx, result)

    all_unique = set()
    for part_idx in range(num_chunks):
        all_unique.update(load_harvest_part(parts_dir, part_idx))
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Harvest EHRSHOT table texts and generate text embeddings."
    )
    parser.add_argument(
        "--stage",
    ...
    parser.add_argument(
        "--cache-dir",
        type=str,
        default="/data/zikun_workspace/input/cache/embeddings/ehrshot",
    ...
    parser.add_argument("--num-workers", type=int, default=120, help="CPU workers for harvesting.")
